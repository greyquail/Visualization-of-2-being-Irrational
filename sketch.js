// Two-arm line visualization for z(theta) = e^(i theta) + e^(i √2 theta)
// Draws all lines swept by the two arms, no shaded circles. [web:111][web:118]

let theta = 0;
let speed = 0.01;

let r1, r2;
const RATIO = Math.SQRT2;      // use Math.PI here if you want the Pi version

// store all line segments the arms sweep out
let segments = [];
...
function draw() {
  ...
  // add current arm positions as line segments
  segments.push({
    a: createVector(0, 0),
    b: createVector(x1, y1)
  });
  segments.push({
  ...
  // draw current arms on top (optional, makes motion clearer)
  stroke(255);
  line(0, 0, x1, y1);
  line(x1, y1, x2, y2);

  // highlight moving endpoint
  fill(255);
  noStroke();
  circle(x2, y2, 4 / zoom);
